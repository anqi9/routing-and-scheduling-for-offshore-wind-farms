                ...
                for wf in range(self.num_WF):
                    ...
                    for v in range(self.num_V):
                        for t in range(self.period):
                            ...
                            for r in range(np.size(R_turbine[v][t][b][wf][0],1)):
                                if x[v,t,b,wf,r].x == 1:
                                    # print ('(v,t,b,wf,r)[%d,%d,%d,%d,%d]'%(v,t,b,wf,r))
                                    # for i in range(len(self.WF_deadline[wf])):
                                    #     if R_turbine[v][t][b][wf][0][i,r] == 1:
                                    #         turbines.append(i)
...
#########################################
    ...
    # def Feasibility(self, L, base, farm, vessel, t):
    #     # time limit
    #     if sum(self.WF_mainTime[farm][tt] for tt in L) \
    #         >= self.time_window[[vessel][t][farm]]:
    #         return False
    # #     # vessel personnel capacity
    # #     # if sum(self.WF_tech[farm][tt][p] for p in self.types for tt in L) > \
    # #     # self.V_tech[vessel]:
    # #         # print ('1', __a, __b)
    # #         # return False
    ...
    #     # for p in self.types:
    #     #     if sum(self.WF_tech[farm][tt][p] for tt in L) >\
    #     #     self.B_tech[base][p][t]:
    #     #         return False
    #     return True
